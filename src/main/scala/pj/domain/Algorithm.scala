...
import pj.typeUtils.opaqueTypes.opaqueTypes.{ID, ODuration, Preference, Time}
...
object Algorithm:
  def makeTheAlgorithmHappen(agenda: Agenda): Result[List[Availability]] =
    ...
    println("=============")
    println(agenda.resources)
    println("=============")
    // Given a structure representing teachers' availabilities

//    val groupedAvailabilitiesList = resourceAvailability
//      .groupBy(_._1)
...
//    println("=============")
//    println(groupedAvailabilitiesList)
//    println("=============")

    val vivas = agenda.vivas
    val duration = agenda.duration

    def allRequiredAvailable(elements: List[Resource], dateTime: Time): Boolean = {
      elements.collect { case teacher: Teacher =>
        teacher.availability.exists { case (start: Time, end: Time) =>
          start.isBefore(dateTime) && end.isAfter(dateTime)
        }
      }.forall(identity)
    }

    val firstAvailableDateTime = LazyList.iterate(LocalDateTime.now)(_ plusDays 1).dropWhile { dateTime =>
      // Chamamos createTime e desempacotamos o Either
      Time.createTime(dateTime.toString) match {
        case Right(localDateTime) =>
          // Verificamos se todos os recursos estão disponíveis na data e hora convertida
          !allRequiredAvailable(agenda.resources, localDateTime)
        case Left(_) =>
          // Em caso de erro ao criar o LocalDateTime, consideramos a data e hora como indisponíveis
          true
      }
    }.headOption

    println(firstAvailableDateTime)
    Right(List.empty[Availability])
